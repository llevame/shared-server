...
function checkParametersValidate(body) {
	return body.username && (body.password || body.facebookAuthToken);
}
...
function credentialsAreValid(user, credentials) {
	...
	if (credentials.password) {
		return validUsername && user.password == credentials.password;
	}

	if (credentials.facebookAuthToken) {
		return (
			validUsername && user.fb.authToken == credentials.facebookAuthToken
		);
	}
}
...
function deleteUser(req, res) {
	userQ
		...
		.then(u => {
			...
			carQ
				.delAllOfUser(req.params.userId)
				.then(() => {
					userQ
			...
			userQ
				...
				.then(updatedUser => {
					carQ
						.getAllOfUser(req.params.userId)
						.then(userCars => {
							let usr = builder.createResponse(
								updatedUser[0],
								userCars
							);
							res.status(200).json(usr);
