...
function postTrip(req, res) {
	...
	Promise.all(q)
		.then(data => {
			let startOfTheDay = moment(req.body.trip.start.timestamp * 1000)
				.startOf('day')
				.unix();
			let endOfTheDay = moment(req.body.trip.start.timestamp * 1000)
				.endOf('day')
				.unix();
			let endT = req.body.trip.start.timestamp;
			let startT = moment(endT * 1000)
				.subtract(30, 'minutes')
				.unix();

			let passengerTripsInDay = tripsInAPeriodOfTime(
				data[0],
				startOfTheDay,
				endOfTheDay
			);
			let driverTripsInDay = tripsInAPeriodOfTime(
				data[1],
				startOfTheDay,
				endOfTheDay
			);
			let passengerTripsInTheLastHalfHour = tripsInAPeriodOfTime(
				data[0],
				startT,
				endT
			);
			...
			var fact = {
				...
				hasLlevameDomain: data[2].email.includes('@llevame.com')
					? true
					: false,
				tripsInTheDayPassenger: passengerTripsInDay.length,
				tripsInTheLastHalfHourPassenger:
					passengerTripsInTheLastHalfHour.length,
				tripsInTheDayDriver: driverTripsInDay.length,
				startDay: moment(req.body.trip.start.timestamp * 1000).format(
					'dddd'
				),
				startTime: parseInt(
					moment(req.body.trip.start.timestamp * 1000).format('H')
				),
				endDay: moment(req.body.trip.end.timestamp * 1000).format(
					'dddd'
				),
				endTime: parseInt(
					moment(req.body.trip.end.timestamp * 1000).format('H')
				),
				cp: [],
				pp: [],
			};
			...
			rulesQ
				...
				.then(rules => {
					rulesModel
						.runTripRules(req, res, rules, fact)
						.then(result => {
							...
							tripsQ
								.add(req.body, req.user.id, cost, currency)
								.then(tripId => {
									let r = [];
									r.push(
										transactionQ.addTransactionTrip(
										...
										.then(transactionsIds => {
											...
											transactionQ
												...
												.then(transId => {
													...
													req.body.trip.applicationOwner =
														req.user.id;
													res
														.status(201)
														.json(
...
function estimateTrip(req, res) {
	...
	let startLocation = [
		req.body.start.address.location.lon,
		req.body.start.address.location.lat,
	];
	let endLocation = [
		req.body.end.address.location.lon,
		req.body.end.address.location.lat,
	];
	...
	Promise.all(q)
		.then(data => {
			let startOfTheDay = moment(req.body.start.timestamp * 1000)
				.startOf('day')
				.unix();
			let endOfTheDay = moment(req.body.start.timestamp * 1000)
				.endOf('day')
				.unix();
			...
			var fact = {
				...
				distance: distance,
				isFirstTrip: data[0].length > 0 ? false : true,
				hasLlevameDomain: data[1].email.includes('@llevame.com')
					? true
					: false,
				tripsInTheDayPassenger: passengerTripsInDay.length,
				tripsInTheLastHalfHourPassenger:
				...
				startDay: moment(req.body.start.timestamp * 1000).format(
					'dddd'
				),
				startTime: parseInt(
					moment(req.body.start.timestamp * 1000).format('H')
				),
				endDay: null,
				endTime: null,
				cp: [],
					...
					rulesModel
						...
						.then(result => {
							...
							res
								.status(200)
								.json(
									builder.createEstimateResponse(
										currency,
										cost
									)
								);
