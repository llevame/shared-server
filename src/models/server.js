	...
	serverQ
		...
		.then(srv => {
			...
			appTokenQ
				...
				.then(() => {
					let token = service.createAppToken(srv);
					let r = builder.createPostResponse(
						srv,
						service.expiration,
						token
					);
...
function updateServer(req, res) {
	...
	serverQ
		...
		.then(server => {
			...
			serverQ
				.update(req.params.serverId, req.body)
				.then(updatedServer => {
...
function pingServer(req, res) {
	...
	serverQ
		.updatePing(id, { lastConnection: knex.fn.now() })
		.then(updatedServer => {
			if (req.user.exp < now) {
				invalidTokensQ
					.add(req.query.token)
					.then(() => {
						...
						let r = builder.createPingResponse(
							updatedServer[0],
							service.expiration,
							token
						);
