...
function getCar(req, res) {
	carQ
		.get(req.params.userId, req.params.carId)
		.then(c => {
			if (!c) {
...
function postCar(req, res) {
	...
	carQ
		.add(req.params.userId, req.body)
		.then(carId => {
...
function updateCar(req, res) {
	...
	carQ
		...
		.then(c => {
			...
			carQ
				.update(req.params.userId, req.params.carId, req.body)
				.then(updatedCar => {
...
function deleteCar(req, res) {
	carQ
		...
		.then(c => {
			...
			carQ
				.del(req.params.userId, req.params.carId)
				.then(() => {
					res.sendStatus(204);
				})
				.catch(err => {
